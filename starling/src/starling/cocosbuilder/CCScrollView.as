...
package starling.cocosbuilder
{
	...
	{
		private var mScrollViewRoot:Sprite = null;
		...
		private var mViewOffsetSize:Point = new Point();
		...
		public override function initWithNodeProperty(nodeInfo:CCNodeProperty):Boolean
		{
			...
			mScrollViewRoot = new Sprite();
			this.addChild(mScrollViewRoot);
		...
		private function onTouch(event:TouchEvent):void
		{
			...
			else if (touch.phase == TouchPhase.MOVED && mIsDown)
			{
				...
				if (mHorizontalMoveEnabled) {
					...
					var viewX:Number = mQuad.bounds.width + mViewOffsetSize.x;
				...
				if (mVerticalMoveEnabled) {
					...
					var viewY:Number = mQuad.bounds.height + mViewOffsetSize.y;
		...
		public function advanceTime(time:Number):void
		{
			...
			var curPosX:Number = mScrollViewRoot.x;
			var curPosY:Number = mScrollViewRoot.y;
			var newPosX:Number = mViewOffsetPosition.x + (mQuad.bounds.width  + mViewOffsetSize.x) * mCurItemPositionX;
			var newPosY:Number = mViewOffsetPosition.y + (mQuad.bounds.height + mViewOffsetSize.y) * mCurItemPositionY;
			if (mVerticalMoveEnabled) {
				if (!IsNearEnough(curPosY, newPosY, 0.1)) {
					mScrollViewRoot.y = newPosY;
				}
			}
			if (mHorizontalMoveEnabled) {
				if (!IsNearEnough(curPosX, newPosX, 0.1)) {
					mScrollViewRoot.x = newPosX;
				}
			}
		}
		
		public function addViewItem(item:DisplayObject):void {
			mScrollViewRoot.addChild(item);
		}
		
		public function setViewOffsetPosition(x:Number, y:Number):void {
			mViewOffsetPosition.setTo(x, y);
			mScrollViewRoot.y = mViewOffsetPosition.y;
			setViewOffsetSize(-this.bounds.width, -this.bounds.height);
		}
		
		private function setViewOffsetSize(x:Number, y:Number):void {
			mViewOffsetSize.setTo(x, y);
			if (mQuad.bounds.width + mViewOffsetSize.x == 0)
				mViewOffsetSize.x = -mQuad.bounds.width  + 1;
			if (mQuad.bounds.height + mViewOffsetSize.y == 0)
				mViewOffsetSize.y = -mQuad.bounds.height + 1;
		}
		
		public function setClipEnabled(clip:Boolean):void {
			this.clipRect = mQuad.bounds;
		}
