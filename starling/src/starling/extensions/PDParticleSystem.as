...
package starling.extensions
{
    ...
    public class PDParticleSystem extends ParticleSystem
    {
        ...
        public function PDParticleSystem(configXML:XML, texture:Texture, configDict:Dictionary = null)
        {
            if (configXML) parseConfig(configXML);
			if (configDict) parseConfigDict(configDict);
            
            var emissionRate:Number = mMaxNumParticles / mLifespan;
			if (configDict) {
				var emissionRateObject:Object = configDict["emissionRate"];
				if (emissionRateObject != null) emissionRate = emissionRateObject as Number;
			}
		...
		private function parseConfigDict(config:Dictionary):void
		{
			// emitter configuration
			var posVar:Point = config["posVar"] as Point;
			mEmitterXVariance = posVar.x;
			mEmitterYVariance = posVar.y;
			mEmitterType = config["emitterMode"] as int;
			// Particle Configuration
			mMaxNumParticles = config["totalParticles"] as int;
			var life:Point = config["life"] as Point;
			mLifespan = life.x;
			mLifespanVariance = life.y;
			var _startSize:Point = config["startSize"] as Point;
			mStartSize = _startSize.x;
			mStartSizeVariance = _startSize.y;
			var _endSize:Point = config["endSize"] as Point;
			mEndSize = _endSize.x;
			mEndSizeVariance = _endSize.y;
			var _angle:Point = config["angle"] as Point;
			mEmitAngle = deg2rad(-_angle.x);
			mEmitAngleVariance = deg2rad(_angle.y);
			var startSpin:Point = config["startSpin"] as Point;
			mStartRotation = deg2rad(startSpin.x);
			mStartRotationVariance = deg2rad(startSpin.y);
			var endSpin:Point = config["endSpin"] as Point;
			mEndRotation = deg2rad(endSpin.x);
			mEndRotationVariance = deg2rad(endSpin.y);
			if (emitterType == EMITTER_TYPE_GRAVITY)
			{
				// gravity configuration
				var gravity:Point = config["gravity"] as Point;
				mGravityX = gravity.x;
				mGravityY = gravity.y;
				var _speed:Point = config["speed"] as Point;
				mSpeed = _speed.x;
				mSpeedVariance = _speed.y;
				var radialAccel:Point = config["radialAccel"] as Point;
				mRadialAcceleration = radialAccel.x;
				mRadialAccelerationVariance = radialAccel.y;
				var tangentialAccel:Point = config["tangentialAccel"] as Point;
				mTangentialAcceleration = tangentialAccel.x;
				mTangentialAccelerationVariance = tangentialAccel.y;
				mMinRadius = 0;
				mMaxRadius = 0;
			...
			{
				// radial configuration 
				var startRadius:Point = config["startRadius"] as Point;
				mMaxRadius = startRadius.x;
				mMaxRadiusVariance = startRadius.y;
				var endRadius:Point = config["endRadius"] as Point;
				mMinRadius = endRadius.x;
				var _rotatePerSecond:Point = config["rotatePerSecond"] as Point;
				mRotatePerSecond = deg2rad(-_rotatePerSecond.x);
				mRotatePerSecondVariance = deg2rad(_rotatePerSecond.y);
			}
			// color configuration
			mStartColor = config["startColor"] as ColorArgb;
			mStartColorVariance = config["startColorVariance"] as ColorArgb;
			mEndColor = config["endColor"] as ColorArgb;
			mEndColorVariance = config["endColorVariance"] as ColorArgb;
			// Blend function
			var blendFactors:Array = config["blendFunc"] as Array;
			mBlendFactorSource = getBlendFunc(blendFactors[0]);
			mBlendFactorDestination = getBlendFunc(blendFactors[1]);
