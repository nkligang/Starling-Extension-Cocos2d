...
package starling.core
{
    ...
    public class Starling extends EventDispatcher
    {
		...
		private var mMessageDisplay:MessageDisplay;
		private var mLineDisplay:LineDisplay;
        ...
        public function showStatsAt(hAlign:String="left", vAlign:String="bottom", scale:Number=1):void
        {
            if (mContext == null)
            {
		...
		/** Indicates if a small statistics box (with FPS, memory usage and draw count) is displayed. */
		public function get showMessage():Boolean { return mMessageDisplay && mMessageDisplay.parent; }
		public function set showMessage(value:Boolean):void
		{
			if (value == showMessage) return;
			
			if (value)
			{
				if (mMessageDisplay) mStage.addChild(mMessageDisplay);
				else               showMessageAt();
			}
			else mMessageDisplay.removeFromParent();
		}
		
		/** Displays the statistics box at a certain position. */
		public function showMessageAt(hAlign:String="left", vAlign:String="top", scale:Number=1):void
		{
			if (mContext == null)
			{
				// Starling is not yet ready - we postpone this until it's initialized.
			...
			else
			{
				...
				{
					mMessageDisplay = new MessageDisplay();
					mMessageDisplay.touchable = false;
					mStage.addChild(mMessageDisplay);
				}
				
				var stageWidth:int  = mStage.stageWidth;
				var stageHeight:int = mStage.stageHeight;
				
				mMessageDisplay.scaleX = mMessageDisplay.scaleY = scale;
				
				if (hAlign == HAlign.LEFT) mMessageDisplay.x = 0;
				else if (hAlign == HAlign.RIGHT) mMessageDisplay.x = stageWidth - mMessageDisplay.width; 
				else mMessageDisplay.x = int((stageWidth - mMessageDisplay.width) / 2);
				
				if (vAlign == VAlign.TOP) mMessageDisplay.y = 0;
				else if (vAlign == VAlign.BOTTOM) mMessageDisplay.y = stageHeight - mMessageDisplay.height;
				else mMessageDisplay.y = int((stageHeight - mMessageDisplay.height) / 2);
			}
			
			function onRootCreated():void
			{
				showMessageAt(hAlign, vAlign, scale);
				removeEventListener(starling.events.Event.ROOT_CREATED, onRootCreated);
			}
		}
		
		public function addMessage(str:String):Boolean
		{
			if (mMessageDisplay == null) return false;
			mMessageDisplay.add(str);
			return true;
		}
		...
		public function drawSingleLine2D(startX:Number, startY:Number, endX:Number, endY:Number, color:uint = Color.WHITE, width:Number = 1):void
		{
			...
			{
				mLineDisplay = new LineDisplay;
				mStage.addChild(mLineDisplay);
			}
			mLineDisplay.drawSingleLine2D(startX, startY, endX, endY, color, width);
		}
